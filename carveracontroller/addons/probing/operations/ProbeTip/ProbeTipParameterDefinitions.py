...
class ProbeTipParameterDefinitions:
    ...
    RepeatOperationCount = ProbeSettingDefinition(
        "L", "Repeat", False, "setting L > 1 will repeat the entire probing operation from the newly found center point"
    )
    ...
    ProbeClearance = ProbeSettingDefinition(
        "J",
        "Probe Clearance",
        False,
        "When Probing a boss, this is added to the X and Y values when moving outside the boss",
    )
